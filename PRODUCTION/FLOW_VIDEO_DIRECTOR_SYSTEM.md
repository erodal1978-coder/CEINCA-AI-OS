...
4. **Cero texto renderizado por el modelo**, salvo overlays estratégicos y pocos (ver sección 2) — la excepción es texto largo/exacto de alta precisión (ej. una dirección de correo completa), que se valida primero en el resultado generado antes de decidir si se mantiene en el prompt o pasa a Edits como excepción puntual de calidad.
5. **Nunca usar el nombre propio del sujeto dentro del prompt.** Usar siempre "el sujeto" / "el hombre" / "el hombre venezolano". Nombrar a una persona real por su nombre dentro de un prompt de generación de video activa el filtro de política de "persona real identificable" en Flow — **validado en producción**: la Escena 1 del guion SAREN/TOTUMA fue rechazada por política exactamente por este motivo, y aprobada al quitar el nombre propio.
6. **Nunca usar la palabra "hiperrealismo" ni "fotorrealista"** en el cierre técnico del prompt — usar en su lugar "calidad cinematográfica profesional". Pedir explícitamente máximo realismo fotográfico de un rostro humano específico alimenta el mismo filtro de persona real.
7. **Especificar siempre la voz:** "voz en español latino neutro con acento venezolano" (para diálogo a cámara y para voz en off/narrador), integrado en la misma línea donde se describe el diálogo.
5. **Descripción real del sujeto en prompts:** lentes grandes translúcidos rosé/marrón tipo aviador con detalle dorado, cabello corto sal y pimienta, chivera corta canosa.
6. **⚠️ NUNCA usar el nombre propio del sujeto dentro del prompt.** Referirse siempre como "el hombre", "el sujeto" o "el protagonista" — nombrar a una persona real por su nombre dentro de un prompt de generación de video activa el filtro de política de Flow sobre generación de personas reales identificables, incluso cuando se trata del propio dueño de la cuenta generando su avatar. **Validado en producción: causó rechazo real de política en la primera generación de campaña.**
7. **⚠️ Evitar la palabra "hiperrealismo"/"fotorrealista" en el prompt.** Pedir explícitamente al modelo maximizar el realismo fotográfico de un rostro humano específico es una segunda señal que alimenta el mismo filtro de persona real. Usar en su lugar "calidad cinematográfica profesional" en el cierre técnico del prompt. **Validado en producción junto con la regla anterior — corrigiendo ambas se resolvió el rechazo de política.**
8. **Especificar siempre el acento/idioma de la voz dentro del prompt:** "voz en español latino neutro con acento venezolano suave" (ajustar el tono según la emoción de la escena: cercano, experto, motivador, etc., pero el acento se mantiene fijo en todas las escenas de una misma pieza).
9. **Escena fusionada, no separada.** Cada escena de Flow es UN solo prompt que integra sujeto/diálogo/voz + b-roll de apoyo (revelado mediante movimiento de cámara: rack focus, paneo, push-in — nunca como una generación aparte) + overlay + música + frame final, todo en un párrafo cinematográfico único. Nunca se entregan "clip principal" y "b-roll" como generaciones independientes para una misma escena narrativa — el b-roll se integra dentro del mismo plano continuo mediante movimiento de cámara.
